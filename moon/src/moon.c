...
typedef union {
	...
	struct {
		...
		double rj;
		unsigned   :2;
		double k;     /* Unsigned is not portable! */
		} fmtb;
	BYTE byts [4];
	long data;
	} wordtype;
...
double fetchreg (unsigned short regnum) {
	if (regnum > 15) {
...
void storereg (unsigned short regnum, double data) {
	if (regnum > 15)
		runtimeerror("simulator error (illegal register code)");
	if (regnum > 0)
		regs[regnum] = data;
	...
	switch (cont) {
			...
			switch (ir.fmta.op) {
				...
				case mod:
					...
					if (rk == 0)
						...
						storereg(ir.fmta.ri,
							(long)fetchreg(ir.fmta.rj) % (long)rk);
						newreg = ir.fmta.ri;
						}
					break;
				...
				case and:
					storereg(ir.fmta.ri,
						(long)fetchreg(ir.fmta.rj) & (long)fetchreg(ir.fmta.rk));
					newreg = ir.fmta.ri;
					break;
		...
		case 'b':
			switch (ir.fmtb.op) {
				...
				case lw:
					storereg(ir.fmtb.ri,
							getmemword(fetchreg(ir.fmtb.rj) + (double) ir.fmtb.k));
					newreg = ir.fmtb.ri;
					break;

				/* lb Ri, K(Rj)  (Load byte) */
				case lb:
					w1 = getmembyte(fetchreg(ir.fmtb.rj) + (double) ir.fmtb.k);
				...
				case sb:
					newmem = fetchreg(ir.fmtb.rj) + (long) ir.fmtb.k;
					putmembyte(newmem, (BYTE) ((long)fetchreg(ir.fmtb.ri) & 255));
					break;

				/* addi Ri, Rj, K  (Add immediate) */
				case addi:
					storereg(ir.fmtb.ri,
						fetchreg(ir.fmtb.rj) + (double) ir.fmtb.k);
					newreg = ir.fmtb.ri;
					break;
				...
				case andi:
					storereg(ir.fmtb.ri,
						(long)fetchreg(ir.fmtb.rj) & (long) ir.fmtb.k);
					newreg = ir.fmtb.ri;
					break;
				...
				case sl:
					storereg(ir.fmtb.ri,
						(long)fetchreg(ir.fmtb.ri) << (long) ir.fmtb.k);
					newreg = ir.fmtb.ri;
					break;
				...
				case ptc:
					if (tracing)
						printf("  Output from putc: %c",
							(int)fetchreg(ir.fmtb.ri));

					else
						printf("%c", (int)fetchreg(ir.fmtb.ri));
					break;
...
*/
	...
	else if (*bp == '-' || *bp == '+' || *bp == '.' || isdigit(*bp)) {
		/* Read a signed decimal integer */
		*t++ = *bp++;
		while (isdigit(*bp) || *bp == '.')
			*t++ = *bp++;
		*t = '\0';
		sscanf(token.symval, "%lf", &token.intval);
		token.kind = T_NUM;
		return;
		}
	else if (*bp == '"') {
...
double getlong (long addr) {
	if (token.kind == T_NUM) {
		double res = token.intval;
		next();
		return res;
		}
 ...
 * in 16 bits.
 */
double getint (long addr) {
	double val = getlong(addr);
//	if (labs(val) <= 32767)
		return val;
